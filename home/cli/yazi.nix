...

{
  ...
  settings = {
    ...
    opener = {
      edit = [
        { run = "${pkgs.neovim}/bin/nvim \"$@\""; desc = "$EDITOR"; block = true; for = "unix"; }
      ];
    };
  };
  ...
  keymap = {
    ...
    manager = {
      prepend_keymap = [
        ...
        { on = "<A-k>"; run = "arrow -5"; desc = "Move cursor up"; }
        { on = "<A-j>"; run = "arrow 5"; desc = "Move cursor down"; }
        { on = [ "f" "g" ]; run = "plugin fg"; desc = "Find file by content (fuzzy match)"; }
        { on = [ "f" "f" ]; run = "filter --smart"; desc = "Filter files"; }
        { on = [ "s" ]; run = "plugin searchjump"; desc = "Flash"; }
